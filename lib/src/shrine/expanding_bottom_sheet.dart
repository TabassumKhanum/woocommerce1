...
class _ExpandingBottomSheetState extends State<ExpandingBottomSheet>
        ...
        child: Column(children: <Widget>[
          Row(children: <Widget>[
            AnimatedPadding(
              ...
              child: Container(height: 22, width: 22, child: Icon(FlutterIcons.shopping_cart_fea),),/*ScopedModel.of<AppStateModel>(context).isCartLoading ?
              Container(height: 22, width: 22, child: CircularProgressIndicator(strokeWidth: 2.0))
                  : Container(height: 22, width: 22, child: Icon(Icons.shopping_basket)),*/
  ...
  Widget _buildCart(BuildContext context, Widget child) {
    ...
    final int numProducts = model.shoppingCart.cartContents.length;
    final int totalCartQuantity = model.count;
...
class _ProductThumbnailRowState extends State<ProductThumbnailRow> {
  ...
  List<CartContent> _internalList;
  ...
  void initState() {
    ...
    _list = _ListModel(
      listKey: _listKey,
      initialItems: ScopedModel.of<AppStateModel>(context).shoppingCart.cartContents,
      removedItemBuilder: _buildRemovedThumbnail,
    );
    _internalList = List<CartContent>.from(_list.list);
  }

  /*CartContent _productWithId(int productId) {
    final AppStateModel model = ScopedModel.of<AppStateModel>(context);
    //final Product product = model.getProductById(productId);
    final CartContent cartContent = model.shoppingCart.cartContents.singleWhere((content) => content.productId == productId);
    assert(cartContent != null);
    return cartContent;
  }*/

  Widget _buildRemovedThumbnail(
      CartContent item, BuildContext context, Animation<double> animation) {
    return ProductThumbnail(animation, animation, item);
  }
  ...
  Widget _buildThumbnail(
    ...
    return ProductThumbnail(
        thumbnailSize, opacity, _list[index]);
  }
  ...
  void _updateLists() {
    ...
    _internalList = ScopedModel.of<AppStateModel>(context).shoppingCart.cartContents;

    final internalSet = Set<CartContent>.from(_internalList);
    final listSet = Set<CartContent>.from(_list.list);


    //final difference = internalSet.where((item) => listContains(listSet, item)).toSet();

    Set<CartContent> difference = {};
    if(internalSet.length > listSet.length) {
      difference = internalSet.where((item) => listContains(listSet, item)).toSet();
    }
    else {
      difference = listSet.where((item) => listContains(internalSet, item)).toSet();
    }
  ...
  listContains(Set<CartContent> listSet, CartContent item) {
    if(listSet.isEmpty) return true;
    else return !listSet.any((element) => element.key == item.key);
  }
}
...
class ExtraProductsNumber extends StatelessWidget {
  ...
  int _calculateOverflow(AppStateModel model) {
    ...
    //List<CartContent> products = productMap.keys.toList();
    int overflow = 0;
    int numProducts = model.shoppingCart.cartContents.length;
    if (numProducts > 3) {
      for (int i = 3; i < numProducts; i++) {
        overflow = numProducts - 3;
      }
    }
    return overflow;
  }
  ...
  Widget _buildOverflow(AppStateModel model, BuildContext context) {
    if (model.shoppingCart.cartContents.length > 3) {
...
typedef RemovedItemBuilder = Widget Function(
    CartContent, BuildContext, Animation<double>);
...
class _ListModel {
  _ListModel(
      {@required this.listKey,
        ...
        Iterable<CartContent> initialItems})
      : assert(listKey != null),
        assert(removedItemBuilder != null),
        _items = List<CartContent>.from(initialItems ?? <CartContent>[]);

  final GlobalKey<AnimatedListState> listKey;
  final RemovedItemBuilder removedItemBuilder;
  final List<CartContent> _items;

  AnimatedListState get _animatedList => listKey.currentState;

  void add(CartContent product) {
    _insert(_items.length, product);
  }

  void _insert(int index, CartContent item) {
    _items.insert(index, item);
    _animatedList.insertItem(index, duration: Duration(milliseconds: 225));
  }

  void remove(CartContent product) {
  ...
  void _removeAt(int index) {
    final CartContent removedItem = _items.removeAt(index);
    if (removedItem != null) {
  ...
  CartContent operator [](int index) => _items[index];

  int indexOf(CartContent item) => _items.indexOf(item);

  List<CartContent> get list => _items;
}
